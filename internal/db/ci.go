...
func GenImageTag(repo *Repository, harborProjectName, commit string) string {
	_ = repo.LoadAttributes()
	return fmt.Sprintf("%s/%s/%s:%s", conf.Docker.Registry, harborProjectName, repo.LowerName, commit)
}
